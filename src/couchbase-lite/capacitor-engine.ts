...
export class CapacitorEngine implements IonicCouchbaseLitePlugin {
  ...
  async Database_Compact(args: DatabaseArgs): Promise<void> {
  ...
  async Engine_Query_Execute(args: EngineQueryExecuteArgs): Promise<ResultSet> {
    ...
    return new ResultSet(args.query, ret.id, args.query.getColumnNames());
  }

  async Query_Execute(args: QueryExecuteArgs): Promise<{ id: string }> {
  ...
  async ResultSet_Cleanup(args: ResultSetCleanupArgs): Promise<void> {
  ...
  Replicator_AddDocumentListener(
    ...
    return IonicCouchbaseLite.Replicator_AddDocumentListener(args);
  }
}
