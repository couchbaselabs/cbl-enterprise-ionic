...
export class Replicator {
  ...
  async start(): Promise<void> {
    ...
    if (
      ...
      await db.getEngine().Replicator_Restart({
    ...
    if (!this.didStartChangeListener) {
      db.getEngine().Replicator_AddChangeListener({
        replicatorId: this.replicatorId,
        callback: (data: ReplicatorChange, err: any) => {
          if (err) {
    ...
    if (!this.didStartDocumentListener) {
      db.getEngine().Replicator_AddDocumentListener({
        replicatorId: this.replicatorId,
        callback: (data: DocumentReplicationRepresentation, err: any) => {
          if (err) {
            console.warn('Replicator ERROR ', err);
            return;
          }
    ...
    await db.getEngine().Replicator_Start({replicatorId: this.replicatorId});
  }
  ...
  async cleanup() {
    ...
    const db = this.config.getDatabase();
    await db.getEngine().Replicator_Cleanup({replicatorId: this.replicatorId});
    this.replicatorId = null;
  }

  stop() {
    const db = this.config.getDatabase();
    return db.getEngine().Replicator_Stop({replicatorId: this.replicatorId});
  }

  resetCheckpoint() {
    const db = this.config.getDatabase();
    return db.getEngine().Replicator_ResetCheckpoint({replicatorId: this.replicatorId});
  }

  getStatus() {
    const db = this.config.getDatabase();
    return db.getEngine().Replicator_GetStatus({replicatorId: this.replicatorId});
  }
